/**
 * Popula o catálogo inicial (mesmos produtos que viviam em `src/catalog.ts` no app)
 * e cria o primeiro usuário OWNER, a partir de `SEED_OWNER_*` no `.env`.
...
const CATALOG: { category: string; rows: Row[] }[] = [
  {
    category: "Linha Aroma",
    rows: [
  ...
  {
    category: "Sabões e tira manchas",
    rows: [
  ...
  { category: "Linha Pet", rows: [["p35", "Eliminador de Odores Mentol 5L", 27.0]] },
  {
    category: "Linha Pasta",
    rows: [
  ...
  {
    category: "Limpadores perfumados",
    rows: [
  ...
  {
    category: "Limpadores específicos",
    rows: [
...
async function seedProducts() {
  ...
  for (const section of CATALOG) {
    for (const [id, name, unitPrice] of section.rows) {
      await prisma.product.upsert({
        where: { id },
        create: { id, name, unitPrice, category: section.category, active: true },
        update: { name, unitPrice, category: section.category },
      });
      count += 1;
    }
  }
  console.log(`Produtos: ${count} sincronizados.`);
}

async function seedOwner() {
  const email = process.env.SEED_OWNER_EMAIL?.trim().toLowerCase();
  const password = process.env.SEED_OWNER_PASSWORD;
  const name = process.env.SEED_OWNER_NAME?.trim() || "Dono";
  if (!email || !password) {
    console.log("SEED_OWNER_EMAIL/SEED_OWNER_PASSWORD não definidos — usuário OWNER não criado.");
    return;
  }
  ...
  console.log(`Usuário OWNER pronto: ${email}`);
}
